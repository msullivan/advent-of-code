...
def score(m, start):
    ...
    return (len({x for x, _ in seen}) - 1)
...
def main(args):
    ...
    ny = len(data)
    starts = (
        [((x, -1), D) for x in range(nx)] +
        [((x, ny), U) for x in range(nx)] +
        [((-1, y), R) for x in range(ny)] +
        [((nx, y), L) for x in range(ny)]
    )
    print(max(score(m, start) for start in starts))
