...
class Group:
    ...
    def from_str(s, boost):
        ...
        return Group(units, hp, damage+boost, init, damage_type, immune, weak)
...
def go(data, boost):
    ...
    for x in data:
        if "units" in x:
            cur.append(Group.from_str(x, boost))
        if x == "Infection:":
            cur = infection
            boost = 0

    old = (sum(x.units for x in immune), sum(x.units for x in infection))
    while immune and infection:
        ...
        new = (sum(x.units for x in immune), sum(x.units for x in infection))
        if old == new:
            break
        old = new
...
def main(args):
    ...
    i = 0
    while True:
        immune, infection = go(data, i)
        if not infection:
            break

        i += 1

    print(sum(x.units for x in immune))
