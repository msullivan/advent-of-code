...
-- This is really bad but the way I did this was sprewing data with
-- Debug.Trace and then finding the first duplicated entry with a tiny
-- python script. This is mostly because of path dependence and not
-- for any good reason. Obviously the right thing would be to plumb
-- the data around and find te duplicate in Haskell but what you gonna
-- do.
--
-- Also it had a nasty memory leak one time I tried to run it and then
-- I tried StrictData and it worked but it seems to work without
-- StrictData when I tried again. IDK.
...
run i instrs ipreg (ip, regs) =
  if ip == 30 then traceShow ("lol", regs !! 5) val else val
  where val = run' i instrs ipreg (ip, regs)
