...
myTurn s@(active, myhp, bosshp, mana, spent) l =
  let myhp_hard = myhp-1
      s'@(_, myhp', bosshp', mana', _) =
        foldr apply (active, myhp_hard, bosshp, mana, spent) active
      active' = filter (\(_,_,t) -> t > 0) $
                map (\(a,b,c) -> (a,b,c-1)) active
  in if myhp_hard <= 0 then [] else if won s' then return (spent, l) else
       do spell <- spells
