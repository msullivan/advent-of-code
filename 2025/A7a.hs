...
drop1 m v =
  let v' = vadd v down
  in Set.fromList $ case Map.lookup v m of
    Just 'S' -> [(v')]
    Just '.' -> [(v')]
...
go m frontier =
  let nf = Set.foldr Set.union Set.empty $ Set.map (drop1 m) frontier
  in if Set.null nf then Set.empty
     else Set.union nf (go m nf)

solve m = sz
  where all = go m (Set.fromList start)
        start = [k | (k, v) <- Map.toList m, v == 'S']
