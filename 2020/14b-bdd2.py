...
# Also contains me just messing around with BDDs.
...
from typing import Tuple, Callable, Set, Iterator, TypeVar, Sequence
...
def var_bdd(var: int) -> Node:
    return Node.new(var, FalseLeaf, TrueLeaf)
...
def count_bdd_orig(bdd: Node, nbits: int) -> int:
...
# implementing as excercise 31 from Knuth 7.1.4
# I did peek though.
def annot_truth_table(
    bdd: Node,
    nbits: int,
    bot: T,
    top: T,
    xs: Sequence[T],
    nxs: Sequence[T],
    add: Callable[[T, T], T],  # open circ
    mul: Callable[[T, T], T],  # closed circ
) -> T:
    def gvar(bdd: Node) -> int:
        return nbits if isinstance(bdd, LeafNode) else bdd.var

    @memoize
    def build(bdd: Node, i: int) -> T:
        if i < gvar(bdd):
            return mul(add(xs[i], nxs[i]), build(bdd, i+1))
        elif bdd is FalseLeaf:
            return bot
        elif bdd is TrueLeaf:
            return top
        else:
            return add(
                mul(nxs[i], build(bdd.false, i+1)),
                mul(xs[i], build(bdd.true, i+1)),
            )
...
def count_bdd(bdd: Node, nbits: int) -> int:
    xs = nxs = (1,) * nbits
    return annot_truth_table(bdd, nbits, 0, 1, xs, nxs, operator.add, operator.mul)
...
def main(args):
    ...
    for i, (mask, val) in reversed(list(enumerate(writes))):
        ...
        # print(f'{count_bdd(bdd, nbits)} {count_bdd2(bdd, nbits)}')
