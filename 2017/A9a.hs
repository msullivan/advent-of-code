...
cleanGarbage ('<':l) = cleanGarbage (stripGarbage l)
cleanGarbage (x:l) = x : cleanGarbage l
cleanGarbage [] = []

parseGroup ('{':r) =
  let (gs, r') = parseGroups r
  in (G gs, r')
parseGroup r = (N, r)
parseGroups ('}':r) = ([], r)
parseGroups r =
  case parseGroup r of
    (g, ',':r') -> let (gs, r'') = parseGroups r' in (g:gs, r'')
    (g, '}':r') -> ([g], r')

score d N = 0
score d (G gs) = d + sum (map (score (d+1)) gs)

main = answer $ score 1 . fst . parseGroup . cleanGarbage . filter (/= '\n')
